    ...
    <PageContainer className="gap-10 pt-16">
      ...
      <section className="grid gap-4 md:grid-cols-[minmax(0,2fr)_minmax(0,3fr)]">
        <Card className="space-y-3">
          <h2 className="text-base font-semibold text-sky-200">Contact</h2>
          <p className="text-base text-zinc-200">{CONTACT_INFO.blurb}</p>
          <dl className="space-y-2 text-base text-zinc-200">
            <div>
        ...
        <Card className="space-y-3">
          ...
          {ABOUT_ME.paragraphs.map((paragraph, i) => (
            <p key={i} className="text-base text-zinc-200">
              {paragraph}
            </p>
          ))}
        </Card>
